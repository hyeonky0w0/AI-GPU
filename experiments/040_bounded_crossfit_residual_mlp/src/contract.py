...
PREDICTION_FEATURES = ["p_915", "p_lgb", "p_cat", "p_lgb_minus_cat",
                       "p_915_minus_lgb", "p_915_minus_cat", "prediction_mean",
                       "prediction_std", "prediction_range"]
MODEL_FEATURES = RAW_FEATURES + PREDICTION_FEATURES
ARCHITECTURE = (256, 128)
SNAPSHOTS = (3, 4, 5)
CAPS = (0.0, 0.05, 0.10, 0.15)
...
def sigmoid_logit_shift(probability, shift):
    import numpy as np
    clipped = np.clip(np.asarray(probability, dtype=np.float64), EPS, 1 - EPS)
    return 1.0 / (1.0 + np.exp(-(np.log(clipped / (1 - clipped)) + shift)))
