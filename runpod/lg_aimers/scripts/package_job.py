...
EXPERIMENTS = {
    ...
    "bounded_residual_dcn": ROOT / "experiments" / "040_bounded_residual_dcn",
}
...
def main() -> None:
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        worker = {"bounded_crossfit_residual_mlp":"train_040.py", "bounded_residual_dcn":"train_040_dcn.py"}.get(mode,"train.py")
        add_file(archive, RUNTIME / "src" / worker, "src/train.py")
        config_name = {"gpu_mlp_smoke":"experiment_036.json", "real_mlp_oof":"experiment_039.json", "bounded_crossfit_residual_mlp":"experiment_040.json", "bounded_residual_dcn":"experiment_040_dcn.json"}.get(mode,"experiment.json")
        config = json.loads((RUNTIME / "config" / config_name).read_text(encoding="utf-8"))
        config["execution_mode"] = mode
        if mode in {"bounded_crossfit_residual_mlp", "bounded_residual_dcn"}:
        ...
        if mode in {"gpu_mlp_smoke", "real_mlp_oof", "bounded_crossfit_residual_mlp", "bounded_residual_dcn"}:
        ...
        for path in sorted(experiment.rglob("*")):
            if (path.is_file() and "__pycache__" not in path.parts and "outputs" not in path.parts
                    and not (mode in {"gpu_mlp_smoke", "real_mlp_oof", "bounded_crossfit_residual_mlp", "bounded_residual_dcn"} and path == experiment / "requirements.txt")):
